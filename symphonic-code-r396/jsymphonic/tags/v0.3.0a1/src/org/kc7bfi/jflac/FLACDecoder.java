...
public class FLACDecoder {
    ...
    // JSymphonic extra fields
    private long jsTotalSamples;
    private int jsBlockSize;
    // Decoder states
     ...
     */
    /**
     * Decode the FLAC file. (Original method)
     * @throws IOException  On read error
     */
    // JSymphonic changes, increment is used to update the GUI
    ...
    /**
     * Decode the FLAC file. (Change for JSymphonic)
     * @throws IOException  On read error
     */
    // JSymphonic changes
    //public void decode() throws IOException {
    public void decode(NWGeneric nwGeneric) throws IOException {
        float value = 0; // JSymphonic variable
        readMetadata();
        double increment = 100.0/(jsTotalSamples/jsBlockSize); // Compute the increment to add at each loop
        ...
        try {
            while (true) {
                ...
                try {
                    ...
                    // TODO to prevent infinite loop, I assume that a bad frames means that the end of the file has been reached.
                    return;
                }
                //    break;
                //case DECODER_END_OF_STREAM :
                ...
                // JSymphonic changes, inform GUI
                try {
                    // Increment the value
                    value += increment;
                    // Inform GUI
                    nwGeneric.sendFileProgressChanged(NWGenericListener.DECODING, (int)value, 0) ;
                }
                catch(Exception e){
                    //Nothing to do if failing because method may be used without JSymphonic...
                }
            }
    ...
    public Metadata readNextMetadata() throws IOException {
        ...
        if (type == Metadata.METADATA_TYPE_STREAMINFO) {
            ...
            // JSymphonic info
            jsBlockSize = streamInfo.getMaxBlockSize();
            jsTotalSamples = streamInfo.getTotalSamples();
