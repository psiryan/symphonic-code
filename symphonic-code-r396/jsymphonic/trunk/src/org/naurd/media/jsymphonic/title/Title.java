...
import org.naurd.media.jsymphonic.playlist.M3uReader;
import org.naurd.media.jsymphonic.playlist.Playlist;
import org.naurd.media.jsymphonic.playlist.PlsReader;
import org.naurd.media.jsymphonic.toolBox.FFMpegToolBox;
import org.naurd.media.jsymphonic.playlist.XspfReader;
...
/* CONSTANT */
    ...
    public static final int OMA = 1; // Format with a number between 1 and 9 can be play on all walkmans
    ...
    public static final int WMMP = 100; // This format is only used for titles transfered on gen1 and gen2 devices
...
/* STATIC METHODS */
    ...
    /**
     * Allows to instanciate a Title or a Playlist using the correct extended class (class Mp3 for MP3 files,...)
     *
     ...
     */
    public static Object getTitleFromFile(java.io.File f) throws UnknowFileTypeException {
        // Get the file name
        ...
        if (Mp3.getFileExtentionsList().contains(extension)){
            ...
            return new Wav(f);
        } else if (M3uReader.getFileExtentionsList().contains(extension)){
            return new M3uReader(f);
        } else if (PlsReader.getFileExtentionsList().contains(extension)){
            return new PlsReader(f);
        } else if (XspfReader.getFileExtentionsList().contains(extension)){
            return new XspfReader(f);
        }
        else{
