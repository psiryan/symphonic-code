 ...
 * This tool box is dedicated to OMGAUDIO database.
 *
...
//        List sortedTitles = sortByArtistAlbumTitleNumber(titles);
     ...
     * @author nicolas_cardoso
     */
    public static void write01TREE22and03GINF22(File omgaudioDir, JSymphonicMap playlists, JSymphonicMap titles) throws Exception{
        /*
        ...
        // Header 122
        WriteTableHeader(rafTable122, "TREE", 2); //Write table header
        WriteClassDescription(rafTable122, "GPLB", 0x30, 0x4010); //Write first class description
        // Compute class's length, need to know how many titles are stored in the playlists
        int titlesNb = 0;

        Iterator it = playlists.values().iterator();
        while(it.hasNext()){
            Playlist playlistTmp = (Playlist) it.next();
            titlesNb += playlistTmp.getTitles().size();
        }
        int class1222Length = titlesNb*0x2 + 0x10;
        class1222Length += 0x10 - (class1222Length % 0x10); // Get an "entire" number
        WriteClassDescription(rafTable122, "TPLB", 0x4040, class1222Length); //Write second class description

        //Reinitialize the iterator on the playlists
        it = playlists.keySet().iterator();
        ...
        int tempPlaylistId, tempKey = 0, TPLBlistCount = 1;
        String playlistName;

        while(it.hasNext()){
            tempPlaylistId = (Integer) it.next(); // Get the playlistId to add
            tempPlaylist = (Playlist) playlists.getValue(tempPlaylistId); // get the playlist
            playlistName = tempPlaylist.getName(); // Get its name

            if(playlistName.length() == 0) playlistName = "unknown playlist"; // Check the name
            tempKey = 0; // Init the key

            // Store the ID of the playlist corresponding to its first title
            GPLBelements.add(TPLBlistCount);
            // Store its name
            playlistNames.add(playlistName);

            // Get the titles info from the list of titles
            Iterator itTit = tempPlaylist.getTitles().iterator();
            while(itTit.hasNext()){
                tempTitle = (Title) itTit.next();

                // Add the title to the list
                titlesIdInTPLBlist.add(titles.get(tempTitle));

                // Increase the key of this playlist with this title
                tempKey += tempTitle.getLength();
                // Increase the number of titles in the TPLB list
                TPLBlistCount++;
            }
        ...
        // Header 322
        WriteTableHeader(rafTable322, "GPIF", 1); //Write table header
        WriteClassDescription(rafTable322, "GPFB", 0x20, (playlists.isEmpty() ? 0 : playlists.size())*0x310 + 0x10); //Write first class description

        //122-Class 1
        WriteClassHeader(rafTable122, "GPLB", playlists.size(), 0x8, playlists.size(), 0); //Write first class header
        //322-Class 1
        WriteClassHeader(rafTable322, "GPFB", playlists.size(), 0x310); //Write first class header

        //Fill in elements in 122-Class 1 & 322-Class 1
        for( int i = 0; i < playlistNames.size(); i++ ) {
            WriteGPLBelement(rafTable122, i+1, (Integer)GPLBelements.get(i));
            WriteGPFBelement(rafTable322, (Integer)titleKeysSorted.get(i), (String)playlistNames.get(i), "", "");
        }
        WriteZeros(rafTable122, 0x4010 - 0x10 - (0x8 * playlistNames.size())); // Fill in the class with zeros

        //122-Class 2
        WriteClassHeader(rafTable122, "TPLB", titlesIdInTPLBlist.size(), 0x2, titlesIdInTPLBlist.size(), 0); //Write first class header

        //Fill in elements in 14-Class 2
        for( int i = 0; i < titlesIdInTPLBlist.size(); i++ ) {
            rafTable122.write(int2bytes((Integer)titlesIdInTPLBlist.get(i), 2));
        }
        WriteZeros(rafTable122, 0x10 - ((titlesIdInTPLBlist.size()*0x2) % 0x10)); // Fill in the class with zeros
    ...
    public static byte[] rtrimZeros(byte[] bytesArray){
        ...
        // If the whole array is filled in with zeros, return an empty array
        if(lastIndex == 0){
            return new byte[0];
        }
